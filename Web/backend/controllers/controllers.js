...
async function getProductList(req, res) {
  try {
    ...
    // }
    let productList = await knex("mytable")
...
async function getProduct(req, res) {
  try {
    ...
    let product = await knex("mytable").where("id", id).first();
    console.log(product)
    return res.status(200).json({
      success: true,
      result: product,
...
async function getFilter(req, res) {
  try {
    let [
      ...
      knex("mytable").groupBy("ram").orderBy("ram", "desc").select("ram"),
      knex("mytable").groupBy("rom").orderBy("rom", "desc").select("rom"),
      knex("mytable")
        .groupBy("battery")
